...
from utils.format_labels import format_body_label, format_contrast_label, format_seq_label, format_view_label, format_alt_stage_label
...
for idx in range(len(dataset["Scan Name"])):
    ...
    y_stage.append(format_alt_stage_label(dataset["ajcc_pathologic_stage"][idx]))
...
y_stage = y_stage.reshape(num_samples, 4)
...
for curr_iter in range(0,50):
    ...
    out_stage = keras.layers.Dense(4,activation="softmax",name="out_stage")

    output_layers = [out_seq, out_view, out_ctrs, out_body, out_stage]
    # output_layers = [out_stage]
    model = cnn3d(output_layers=output_layers)
    model.summary()
    ...
    labels = np.arange(0,4,1,dtype=int)
    label_name = ['Stage I/A/B', 'Stage II/A/B', 'Stage III/A/B/C', 'Stage IV/A/B']
    ...
    performance_pd.to_csv(os.path.join(output_dir,"multi_cnn3d{}.csv".format(curr_iter+5)))
